<?php
	class Formato{
		...
		public static function noDinero($dinero){
			if(strpos($dinero,"(")!==false){
				$dinero = substr($dinero,0,strpos($dinero,"("));
			}
			
			return trim(str_replace("$","",str_replace(" ","",str_replace(",","",$dinero))));
		}
		...
		public static function fechaFutura($fecha, $dias = 0){
	        if(strpos($fecha,"-")>0){
	            $tiempo = mktime(0,0,0,substr($fecha,5,2),substr($fecha,8,2),substr($fecha,0,4));
	        }
			
			if(strpos($fecha,"/")>0){
				$tiempo = mktime(0,0,0,substr($fecha,3,2),substr($fecha,0,2),ubstr($fecha,6,4));
	        }
		...
		public static function ultimoDiaMes($mes){
			return date("t",mktime(0,0,0,$mes,1,date("Y")));
		}
